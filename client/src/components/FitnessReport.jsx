...
import formatReportPayload from '../utils/formatReportPayload';
...
export const generateFitnessReportPDF = async (payload, filename = 'Fitness_Report') => {
  ...
  // Column widths: TestName | L1 | L2 | L3 | L4 | L5 | L6 | L7 | My Score | Indicator | Feedback
  const SCORE_COLS = {
    test:      38,
    l1:        9,  l2: 9, l3: 9, l4: 9, l5: 9, l6: 9, l7: 9,
    score:     22,
    indicator: 18,
    feedback:  CONTENT_W - 38 - 9*7 - 22 - 18
  };
  ...
  const TEST_ROW_H = 8; // each test = 2 sub-rows (Current + Previous) = 4mm each

  fitnessScores.forEach((test, tIdx) => {
    const rowBg     = tIdx % 2 === 0 ? C.white : C.tableBg;
    const subRowH   = TEST_ROW_H / 2;
    const testRowY  = y;

    // ── Test name cell (spans both sub-rows)
    filledRect(doc, MARGIN, testRowY, SCORE_COLS.test, TEST_ROW_H, rowBg, C.borderBlue, 0.2);
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...C.black);
    doc.text(test.testName, MARGIN + 1.5, testRowY + 3);
    // "Current" sub-label
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(5.5);
    doc.setTextColor(...C.darkGray);
    doc.text('Current',  MARGIN + 3, testRowY + subRowH + 2.5);
    doc.text('Previous', MARGIN + 3, testRowY + TEST_ROW_H - 1);

    // ── L1–L7 dot cells (one per level, coloured circle in the test's level)
    cx = MARGIN + SCORE_COLS.test;
    ['L1','L2','L3','L4','L5','L6','L7'].forEach((lvl) => {
      // Full cell background
      filledRect(doc, cx, testRowY, 9, TEST_ROW_H, rowBg, C.borderBlue, 0.2);

      // Active level: filled circle
      if (test.level === lvl) {
    ...
    // ── My Score cell (spans both sub-rows)
    filledRect(doc, cx, testRowY, SCORE_COLS.score, TEST_ROW_H, rowBg, C.borderBlue, 0.2);
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...C.textBlue);
    const scoreStr = test.currentScore !== 'N/A' ? test.currentScore : 'N/A';
    doc.text(scoreStr, cx + 1.5, testRowY + 3.2);
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...C.darkGray);
    doc.text(test.previousScore !== 'N/A' ? test.previousScore : '—', cx + 1.5, testRowY + TEST_ROW_H - 1.5);
    cx += SCORE_COLS.score;
    ...
    const indW   = doc.getTextWidth(indStr);
    doc.text(indStr, cx + (SCORE_COLS.indicator - indW) / 2, testRowY + TEST_ROW_H / 2 + 1.2);
    cx += SCORE_COLS.indicator;

    // ── Feedback cell (word-wrap)
    ...
    doc.text(feedLines.slice(0, 2), cx + 1, testRowY + 3.2);  // max 2 lines
  ...
  recommendations.dietary.forEach((item, idx) => {
    ...
    // Colour the category label (before the dash)
    const labelEnd = item.indexOf(' - ');
    if (labelEnd > 0) {
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...C.greenDark);
      doc.text(`${alpha}. ${item.substring(0, labelEnd)}`, MARGIN + 3, y);
    }
 ...
 *  4. Wraps everything in try/catch/finally (isExporting always resets)
 *
 * Props:
 *  @prop {Object}      student      - Student document
 *  @prop {Object}      currentPerf  - Current term Performance document
 *  @prop {Object|null} prevPerf     - Previous term Performance document
 *  @prop {Object}      institute    - { name, city, state }
 *  @prop {boolean}     disabled     - External disable (e.g. no perf data)
 *  @prop {string}      [className]  - Extra Tailwind classes on the button
 */
const FitnessReportExportButton = ({
  ...
  const handleExport = async () => {
    ...
    try {
      // ── Build the clean payload via Phase 2 utility ──────────────────────
      const payload = formatReportPayload(student, currentPerf, prevPerf, institute);

      // ── Construct a safe filename ────────────────────────────────────────
      const safeName = (student.name || 'Student').replace(/\s+/g, '_');
      const filename  = `${safeName}_Fitness_Report_${payload.term}`;
